...
const SearchResult = () => {
  ...
  const dish =
    resultInfo?.data?.cards[1]?.groupedCard?.cardGroupMap?.DISH?.cards?.filter(
      (item) => {
        return (
          item?.card?.card?.["@type"] ===
  ...
  const restaurant =
    resultInfo?.data?.cards[1]?.groupedCard?.cardGroupMap?.RESTAURANT?.cards?.filter(
      (item) => {
        return (
          item?.card?.card?.["@type"] ===
    ...
    <div className="lg:w-7/12 mx-auto">
      {resultInfo?.data?.cards[1]?.groupedCard?.cardGroupMap?.DISH ? (
        <div className="bg-orange-50 lg:flex lg:flex-wrap lg:justify-center gap-3 py-4 ">
          {dish.map((item) => {
            return <DishCard key={item.card.card.info.id} item={item} />;
          })}
        </div>
      ) : (
        <div className="bg-orange-50 lg:flex lg:flex-wrap lg:justify-center gap-3 py-4 ">
